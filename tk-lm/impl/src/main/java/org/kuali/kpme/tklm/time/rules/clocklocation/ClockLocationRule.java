...
@Entity
@Table(name = "TK_CLOCK_LOCATION_RL_T")
public class ClockLocationRule extends TkRule implements DepartmentalRule, ClockLocationRuleContract {
    ...
    public static final ImmutableList<String> BUSINESS_KEYS = new ImmutableList.Builder<String>().add(KeyFields.DEPT).add(KeyFields.WORK_AREA).add(KeyFields.JOB_NUMBER).add(KeyFields.PRINCIPAL_ID).add(KeyFields.GROUP_KEY_CODE).build();

    @PortableSequenceGenerator(name = "TK_CLOCK_LOCATION_RL_S")
    @GeneratedValue(generator = "TK_CLOCK_LOCATION_RL_S")
    @Id
    ...
    @Column(name = "WORK_AREA", nullable = false, precision = 10)
    ...
    @Column(name = "JOB_NUMBER", nullable = false, precision = 3)
    ...
    public ImmutableMap<String, Object> getBusinessKeyValuesMap() {
        return new ImmutableMap.Builder<String, Object>().put(KeyFields.DEPT, this.getDept()).put(KeyFields.WORK_AREA, this.getWorkArea()).put(KeyFields.JOB_NUMBER, this.getJobNumber()).put(KeyFields.PRINCIPAL_ID, this.getPrincipalId()).put(KeyFields.GROUP_KEY_CODE, this.getGroupKeyCode()).build();
    }
    ...
    public String getUniqueKey() {
        String clockLocKey = getDept() + "_" + getPrincipalId() + "_" + (getJobNumber() != null ? getJobNumber().toString() : "") + "_" + (getWorkArea() != null ? getWorkArea().toString() : "") + "_" + (getGroupKeyCode() != null ? getGroupKeyCode().toString() : "");
        return clockLocKey;
    }
