...
public class AssignmentRow implements Serializable {
	...
	public void addToAmount(int index, BigDecimal amount) {
		if (amount != null) {
			if (0 <= index && index < getAssignmentColumns().size()) {
				...
				BigDecimal newAmount = assignmentColumn.getAmount().add(amount, TkConstants.MATH_CONTEXT);
	...
	public void addToTotal(int index, BigDecimal total) {
		if (total != null) {
			if (0 <= index && index < getAssignmentColumns().size()) {
				...
				BigDecimal newTotal = assignmentColumn.getTotal().add(total, TkConstants.MATH_CONTEXT);
	...
	public void addWeeklyTotal(int index, int weekSize) {
		...
		for (ListIterator<AssignmentColumn> iterator = getAssignmentColumns().listIterator(index); iterator.hasPrevious() && iterator.previousIndex() >= (index - weekSize); ) {
			...
			weeklyAmount = weeklyAmount.add(assignmentColumn.getAmount(), TkConstants.MATH_CONTEXT);
			weeklyTotal = weeklyTotal.add(assignmentColumn.getTotal(), TkConstants.MATH_CONTEXT);
		}
		...
		if (!getAssignmentColumns().isEmpty()) {
			...
			BigDecimal periodTotal = assignmentColumn.getTotal().add(weeklyTotal, TkConstants.MATH_CONTEXT);
			assignmentColumn.setTotal(periodTotal);
			
			BigDecimal periodAmount = assignmentColumn.getAmount().add(weeklyAmount, TkConstants.MATH_CONTEXT);
