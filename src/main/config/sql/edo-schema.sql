...
--------------------------------------------------------
  ...
  ALTER TABLE EDO.EDO_ITEM_T ADD CONSTRAINT EDO_ITEM_TP1 PRIMARY KEY (ITEM_ID);
--------------------------------------------------------
...
-----------------------------------------------------------------------------
-- DDL for edo_item_count_v

create or replace force view edo.edo_item_count_v
(
    doc_count,
    checklist_item_id,
    checklist_item_name,
...
as
  ...
  from     edo_item_v i
    inner join edo_checklist_item_t s
      on i.checklist_item_id = s.checklist_item_id
  group by i.checklist_item_id,
    checklist_item_name,
    dossier_id,
    s.checklist_section_id;
