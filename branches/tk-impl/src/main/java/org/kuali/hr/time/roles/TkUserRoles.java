...
public class TkUserRoles implements UserRoles {
	...
	private Map<String, TkRole> payrollProcessorRolesDept = new HashMap<String, TkRole>();
	private Map<String, TkRole> payrollProcessorDelegateRolesDept = new HashMap<String, TkRole>();
	...
	/**
	 * Constructor that takes a list of all roles that will be encapsulated by
	 * this object.
	 * 
	 * @param roles
	 */
	 ...
	 * 
	 * @param roles
	 *            A List of TkRole objects for the current user.
	 */
		...
		for (TkRole role : roles) {
			if (role.getRoleName().equals(TkConstants.ROLE_TK_APPROVER)) {
				approverRoles.put(role.getWorkArea(), role);
					...
					TkConstants.ROLE_TK_APPROVER_DELEGATE)) {
				...
				if (!StringUtils.isEmpty(role.getChart())) {
					...
					List<Department> ds = TkServiceLocator
							.getDepartmentService().getDepartments(
									role.getChart(), TKUtils.getCurrentDate());
					for (Department d : ds) {
						orgAdminRolesDept.put(d.getDept(), role);
					}
				}
			} else if (StringUtils.equals(role.getRoleName(),
			...
			} else if (role.getRoleName()
					.equals(TkConstants.ROLE_TK_DEPT_ADMIN)) {
				...
				payrollProcessorRolesDept.put(role.getDepartment(), role);
			} else if (role.getRoleName().equals(
					IUTkConstants.ROLE_TK_PY_PROCESSOR_DELEGATE)) {
				payrollProcessorDelegateRolesDept.put(role.getDepartment(),
						role);
			} else {
				throw new RuntimeException("Invalid Role."); // TODO: Maybe we
																// want to just
																// ignore this
																// exception.
			}
	...
	public boolean isAnyApproverActive() {
		return this.approverRoles.size() > 0
				|| this.approverDelegateRoles.size() > 0;
	}
	...
	public boolean isApproverForTimesheet(TimesheetDocument doc) {
		...
		for (Assignment assignment : assignments) {
			if (this.approverRoles.containsKey(assignment.getWorkArea())
					|| this.approverDelegateRoles.containsKey(assignment
							.getWorkArea())) {
				return true;
			}
		}
	...
	public boolean isDocumentWritable(TimesheetDocument document) {
		...
		writable |= (StringUtils.equals(this.principalId,
		...
		if (!writable) {
			...
			for (Assignment assignment : assignments) {
				...
				writable |= this.getPayrollProcessorWorkAreas().contains(wa);
			}
		}
	...
	public boolean isDocumentWritable(String documentId) {
		return isDocumentWritable(TkServiceLocator.getTimesheetService()
	...
	public boolean isDocumentReadable(String documentId) {
		...
		if (documentId != null) {
			return isDocumentReadable(TkServiceLocator.getTimesheetService()
					.getTimesheetDocument(documentId));
		}
	...
	public boolean isDocumentReadable(TimesheetDocument document) {
		...
		// Owner
		readable |= StringUtils.equals(this.principalId,
		...
		if (!readable) {
			...
			for (Assignment assignment : assignments) {
				...
				readable |= this.reviewerRoles.containsKey(wa);
				readable |= this.getPayrollProcessorWorkAreas().contains(wa);
			}
		}
	...
	public boolean isApproverForPerson(String principalId) {
		List<Assignment> lstAssignment = TkServiceLocator
				.getAssignmentService().getAssignments(principalId,
						TKUtils.getCurrentDate());

		for (Assignment assignment : lstAssignment) {
			if (TKContext.getUser().getCurrentRoles().getApproverWorkAreas()
					.contains(assignment.getWorkArea())) {
				return true;
			}
		}
	...
	public boolean canSubmitTimesheet(TimesheetDocument doc) {
		...
		if (this.isApproverForTimesheet(doc)
				|| this.isPayrollProcessorForTimesheet(doc)) {
			return true;
		}
	...
	@Override
	public boolean isTimesheetPayrollProcessor() {
		return this.payrollProcessorRolesDept.size() > 0
				|| this.payrollProcessorDelegateRolesDept.size() > 0;
	}

	@Override
	public boolean isAnyPayrollProcessorActive() {
		return this.payrollProcessorRolesDept.size() > 0
				|| this.payrollProcessorDelegateRolesDept.size() > 0;
	}

	@Override
	public boolean isPayrollProcessorForTimesheet(TimesheetDocument doc) {
		boolean payrollProcessor = false;
		...
		for (Assignment assignment : assignments) {
			if (this.getPayrollProcessorWorkAreas().contains(assignment.getWorkArea())) {
				return true;
			}
		}
	...
	@Override
	public boolean isPayrollProcessorForTimesheet(String docId) {
		boolean payrollProcessor = false;
		...
		if (docId != null) {
			TimesheetDocument doc = TkServiceLocator.getTimesheetService()
					.getTimesheetDocument(docId);
			if (doc != null)
				payrollProcessor = isPayrollProcessorForTimesheet(doc);
		}
	...
	@Override
	public Set<Long> getPayrollProcessorWorkAreas() {
		Set<Long> workAreas = new HashSet<Long>();
		List<WorkArea> wa = null;

		for (String dept : payrollProcessorRolesDept.keySet()) {
			wa = TkServiceLocator.getWorkAreaService().getWorkAreas(dept,
					TKUtils.getCurrentDate());
			
			for (WorkArea workArea : wa) {
				workAreas.add(workArea.getWorkArea());
			}
		}

		for (String dept : payrollProcessorDelegateRolesDept.keySet()) {
			wa = TkServiceLocator.getWorkAreaService().getWorkAreas(dept,
					TKUtils.getCurrentDate());
	...
	@Override
	public Set<String> getPayrollProcessorDepartment() {

		if (payrollProcessorDelegateRolesDept.size() > 0) {
			payrollProcessorRolesDept.putAll(payrollProcessorDelegateRolesDept);
		}
		return payrollProcessorRolesDept.keySet();

	}

	@Override
	public boolean isPayrollProcessorForPerson(String principalId) {
		List<Assignment> lstAssignment = TkServiceLocator
				.getAssignmentService().getAssignments(principalId,
						TKUtils.getCurrentDate());

		for (Assignment assignment : lstAssignment) {
			if (TKContext.getUser().getCurrentRoles()
					.getPayrollProcessorWorkAreas()
					.contains(assignment.getWorkArea())) {
				return true;
			}
		}
