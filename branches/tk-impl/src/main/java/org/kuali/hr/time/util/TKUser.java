...
public class TKUser {
	...
	public Multimap<String, Long> getReportingPayrollProcessorDepartments(){
		UserRoles userRoles = this.getCurrentRoles();
        // see the comment in the getDeptWorkAreasByWorkAreas() for the explanation of Multimap
		Multimap<String, Long> reportingPayrollProcessorDepartments = HashMultimap.create();

		Set<String> depts = new HashSet<String>();
		depts.addAll(userRoles.getPayrollProcessorDepartment());
		
        if (depts.size() > 0) {
        	reportingPayrollProcessorDepartments.putAll(TkServiceLocator.getTimeApproveService().getDeptWorkAreasByDepts(depts));
        }
		
		return reportingPayrollProcessorDepartments;
	}
	...
	public Set<Long> getReportingWorkAreas(){
		...
		reportingWorkAreas.addAll(userRoles.getPayrollProcessorWorkAreas());
    ...
    public SortedSet<Long> getWorkAreasFromUserRoles() {
        ...
        workAreas.addAll(this.getCurrentRoles().getPayrollProcessorWorkAreas());
	...
	public boolean isPayrollProcessor() {
		UserRoles userRoles = getCurrentRoles();
		return userRoles.getPayrollProcessorWorkAreas().size() > 0;
	}
