...
public class TkWorkflowTimesheetAttribute implements RoleAttribute {
	...
	private static final Logger LOG = Logger
			.getLogger(TkWorkflowTimesheetAttribute.class);

	@Override
	public List<String> getQualifiedRoleNames(String roleName,
			DocumentContent documentContent) {
		List<String> roles = new ArrayList<String>();
		Long routeHeaderId = documentContent.getRouteContext().getDocument()
				.getRouteHeaderId();
		TimesheetDocument timesheetDocument = TkServiceLocator
				.getTimesheetService().getTimesheetDocument(
						routeHeaderId.toString());
	...
	public List<Role> getRoleNames() {
		...
		throw new UnsupportedOperationException(
				"Not supported in TkWorkflowTimesheetAttribute");
	}
	...
	public ResolvedQualifiedRole resolveQualifiedRole(
		...
		try {
			...
			if (pos > -1) {
				String subs = qualifiedRole.substring(pos + 1,
						qualifiedRole.length());
		...
		if (workAreaNumber == null) {
			throw new RuntimeException(
					"Unable to resolve work area during routing.");
		}
		...
		TimesheetDocument timesheetDocument = TkServiceLocator
				.getTimesheetService().getTimesheetDocument(
						routeHeaderId.toString());
		WorkArea workArea = TkServiceLocator.getWorkAreaService().getWorkArea(
				workAreaNumber, timesheetDocument.getAsOfDate());

		List<TkRole> roles = new ArrayList<TkRole>();
		if (roleName.equals("TK_APPROVER")) {
			List<TkRole> approvers = roleService.getWorkAreaRoles(
					workAreaNumber, roleName, TKUtils.getCurrentDate());
			List<TkRole> approverDelegates = roleService.getWorkAreaRoles(
					workAreaNumber, TkConstants.ROLE_TK_APPROVER_DELEGATE,
		...
		if (roleName.equals("TK_PY_PROCESSOR")) {
			List<TkRole> payrollProcessor = roleService.getWorkAreaRoles(
					workAreaNumber, roleName, TKUtils.getCurrentDate());
			List<TkRole> payrollProcessorDelegates = roleService.getWorkAreaRoles(
					workAreaNumber, IUTkConstants.ROLE_TK_PY_PROCESSOR_DELEGATE,
					TKUtils.getCurrentDate());

			roles.addAll(payrollProcessor);
			roles.addAll(payrollProcessorDelegates);
		}
		...
		for (TkRole role : roles) {
			...
			if (StringUtils.isEmpty(role.getPrincipalId())) {
				...
				List<Job> lstJobsForPosition = TkServiceLocator.getJobSerivce()
						.getActiveJobsForPosition(
								positionNumber,
								timesheetDocument.getPayCalendarEntry()
		...
		if (principals.size() == 0)
			throw new RuntimeException(
					"No principals to route to. Push to exception routing.");

		rqr.setRecipients(principals);
		rqr.setAnnotation("Dept: " + workArea.getDept() + ", Work Area: "
