...
public class LookupDaoOjb extends PlatformAwareDaoBaseOjb implements LookupDao {
    ...
    //HACK to make lookups work with appropriate data source
    //Needed to use module service but there is no logical cutover for these objects
    //and the downstream changes in the KNS config look difficult
    
    private PersistenceBrokerTemplate getPersistenceTemplate(Class businessObject){
    	if(businessObject.equals(PayType.class) || businessObject.equals(PayGrade.class) ||
    			businessObject.equals(Department.class) || businessObject.equals(HolidayCalendar.class) || businessObject.equals(Job.class) ||
    			businessObject.equals(Location.class) || businessObject.equals(Position.class) || businessObject.equals(SalGroup.class) ||
    			businessObject.equals(AccrualCategory.class) || businessObject.equals(TimeOffAccrual.class) ||
    			businessObject.equals(HolidayCalendarDateEntry.class) || businessObject.equals(Chart.class) || businessObject.equals(Account.class) || 
    			businessObject.equals(SubAccount.class) || businessObject.equals(ObjectCode.class) || businessObject.equals(SubObjectCode.class) ||
    			businessObject.equals(ProjectCode.class) || businessObject.equals(Organization.class) ){
    		return edu.iu.hr.time.service.base.TkServiceLocator.getSudsDao().getPersistenceBrokerTemplate();
    	}
    ...
    private Collection executeSearch(Class businessObjectClass, Criteria criteria, boolean unbounded) {
    	...
    	try {
    		...
    		if (!unbounded && (searchResultsLimit != null)) {
    			matchingResultsCount = new Long(getPersistenceTemplate(businessObjectClass).getCount(QueryFactory.newQuery(businessObjectClass, criteria)));
    			LookupUtils.applySearchResultsLimit(businessObjectClass, criteria, getDbPlatform());
    		}
    		if ((matchingResultsCount == null) || (matchingResultsCount.intValue() <= searchResultsLimit.intValue())) {
    			matchingResultsCount = new Long(0);
    		}
    		searchResults = getPersistenceTemplate(businessObjectClass).getCollectionByQuery(QueryFactory.newQuery(businessObjectClass, criteria));
    ...
    public Long findCountByMap(Object example, Map formProps) {
        ...
        return new Long(getPersistenceTemplate(example.getClass()).getCount(query));
    }
    ...
    public Object findObjectByMap(Object example, Map formProps) {
    	if ( persistenceStructureService.isPersistable(example.getClass())) {
	    	...
	    	// execute query and return result list
	    	Query query = QueryFactory.newQuery(example.getClass(), criteria);
	    	return getPersistenceTemplate(example.getClass()).getObjectByQuery(query);
    	}
    	return null;
    }
