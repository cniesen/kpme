...
<c:choose>
    <c:when test='${Form.roleName == "TK_PY_PROCESSOR"}'>
    	<c:set var="tabid" value="payrollProcessor" scope="request"/>
    </c:when>
    <c:otherwise>
    	<c:set var="tabid" value="approvals" scope="request"/>
    </c:otherwise>
</c:choose>
 <tk:tkHeader tabId="${tabid}">
        ...
        <tr>
            <td class="left">
                ...
                <label for="search value">
           			<input id="searchValue" name="searchValue" type="text" value="${Form.searchTerm}" placeholder="enter at least 5 chars" />
...
<display:table name="${Form.approvalRows}" requestURI="TimeApproval.do?methodToCall=loadApprovalTab&roleName=${Form.roleName}" excludedParams="*"
               pagesize="20" id="row"
    ...
    <display:column title="Name" sortable="true" sortName="principalName" style="${nameStyle}">
        ...
        <a href="Admin.do?${row.timesheetUserTargetURLParams}&targetUrl=PersonInfo.do&returnUrl=TimeApproval.do&roleName=${Form.roleName}">${row.name}</a> (${row.principalId})
        <br/>${row.clockStatusMessage}
        <br/>
    </display:column>
    <display:column title="Document ID" sortable="true" sortName="documentId">
        <a href="Admin.do?${row.timesheetUserTargetURLParams}&targetUrl=TimeDetail.do%3FdocumentId=${row.documentId}&returnUrl=TimeApproval.do&roleName=${Form.roleName}">${row.documentId}</a>
