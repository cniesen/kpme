	...
	public List<KeyValue> getKeyValues(ViewModel model) {
		...
		if (model instanceof MissedPunchForm) {
	        ...
	        if (StringUtils.isNotBlank(clockAction)) {
	            ...
	            ClockLog lastClock = TkServiceLocator.getClockLogService().getLastClockLog(TKContext.getTargetPrincipalId());
	            ...
	            if (lastClock != null) {
	            	Job jobObj = TkServiceLocator.getJobService().getJob(TKContext.getTargetPrincipalId(), lastClock.getJobNumber(), LocalDate.now());
		            ...
		            if (!TkServiceLocator.getSystemLunchRuleService().getSystemLunchRule(LocalDate.now()).getShowLunchButton()) {
		            	...
		            	if (TkServiceLocator.getDepartmentLunchRuleService().getDepartmentLunchRule(department, workArea, TKContext.getTargetPrincipalId(), jobNumber, LocalDate.now()) != null) {
			            	availableActions.remove(TkConstants.LUNCH_OUT);
			            	availableActions.remove(TkConstants.LUNCH_IN);
		            	}
		            }
