...
<c:choose>
	...
	<c:otherwise>
		<c:if test="${!hidden}">
			<html:text name="StrutsActionForm" property="${hourName}" styleId="${hourName}" size="2" maxlength="2" />:
			<html:text name="StrutsActionForm" property="${minuteName}" styleId="${minuteName}" size="2" maxlength="2" />
