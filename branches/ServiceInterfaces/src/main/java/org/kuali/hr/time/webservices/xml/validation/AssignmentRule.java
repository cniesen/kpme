...
public class AssignmentRule {
	...
	public boolean validateAssignmentObject(Assignment assignment) throws IllegalArgumentException{
		boolean result  = false;
		if (assignment != null) {
		
			if(!this.validateWorkArea(assignment)){
				throw new IllegalArgumentException("Invalid WorkArea :"+assignment.getWorkArea());
			}		
			if( !this.validateJob(assignment)){
				throw new IllegalArgumentException("Invalid Job :"+assignment.getJob());				
			}
			if(! this.validateTask(assignment)){				
				throw new IllegalArgumentException("Invalid Task :"+assignment.getTask());
			}
			result = true;
		}
		return result;
