...
public class PayTypeServiceImpl implements PayTypeService {
	...
	public PayType getPayType(String payType, Date effectiveDate) {
		...
		payTypeObj = SpringContext.getBean(PayTypeDao.class).getPayType(payType, effectiveDate);
		if (payTypeObj != null) {
			PayCalendar payCalendar = SpringContext.getBean(PayCalendarService.class).getPayCalendarByGroup(payTypeObj.getCalendarGroup());
	...
	public boolean addPayTypes(List<PayType> payTypes) 	throws TkException{
		...
		for (PayType newPayType : payTypes) {
			try {
				...
				PayType oldPayType = payTypeDao.getPayType(newPayType.getPayType(), new Date(new java.util.Date().getTime()));
					// if no .. save it
				if (oldPayType == null) {
					...
					oldPayType.setCalendarGroup (newPayType.getCalendarGroup());
					...
					oldPayType.setHolidayCalendarGroup(newPayType.getHolidayCalendarGroup());
